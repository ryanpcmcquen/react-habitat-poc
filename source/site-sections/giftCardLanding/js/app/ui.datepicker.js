		...
		_checkOffset: function(inst, offset, isFixed) {
			...
			offset.top -=
				offset.top + dpHeight > viewHeight && viewHeight > dpHeight
					? Math.abs(
							offset.top + dpHeight + inputHeight * 2 - viewHeight
						)
					: 0;
	   ...
	   @param  duration  string - the duration over which to close the date picker */
		_hideDatepicker: function(input, duration) {
			...
			if (this._datepickerShowing) {
				duration =
					duration != null ? duration : this._get(inst, "duration");
	   ...
	   @return  number - the number of the week within the year that contains this date */
		iso8601Week: function(date) {
			...
			if (firstDay < 4 && checkDate < firstMon) {
				...
				checkDate > new Date(checkDate.getFullYear(), 12 - 1, 28)
			) {
				...
				date == null
					...
					: typeof date == "string"
						...
						: typeof date == "number"
							...
							: date;
			date =
				date && date.toString() == "Invalid Date" ? defaultDate : date;
			if (date) {
				date.setHours(0);
				date.setMinutes(0);
				date.setSeconds(0);
									...
									: unselectable
										...
										: '<a class="ui-state-default' +
											...
											(printDate.getTime() >=
												currentDate.getTime() &&
											printDate.getTime() <=
												endDate.getTime() // in current range
												? " ui-state-active"
												: "") + // highlight selected day
											'" href="#">' +
											printDate.getDate() +
