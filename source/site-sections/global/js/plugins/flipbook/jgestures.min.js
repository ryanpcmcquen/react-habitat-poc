...
(function(c) {
		...
		function(l, k) {
			jQuery.event.special[k] = {
				setup: function() {
					...
					var n;
					if (
						!p.data("ojQueryGestures") ||
						!p.data("ojQueryGestures")[o]
					) {
	...
	function d(k) {
		...
		if (k.touches) {
			...
			n[0].moved = Math.sqrt(
				Math.pow(Math.abs(n[0].startX), 2) +
					Math.pow(Math.abs(n[0].startY), 2)
			);
		}
		return {
			type: k.type || null,
			...
			duration: k.duration
				? k.duration
				: k.startMove.timestamp ? l - k.timestamp : null,
			...
			switch (k) {
				...
				case "shakeupdown":
					...
					if (
						...
						(q.accelerationIncludingGravity.x >
							m.leftright.sensitivity ||
							q.accelerationIncludingGravity.x <
								-1 * m.leftright.sensitivity)
					) {
						p.push("leftright");
						p.push("x-axis");
					}
					if (
						...
						(q.accelerationIncludingGravity.y >
							m.frontback.sensitivity ||
							q.accelerationIncludingGravity.y <
								-1 * m.frontback.sensitivity)
					) {
						p.push("frontback");
						p.push("y-axis");
					}
	...
	function j(r) {
		...
		var y =
			...
			Math.abs(m.oStartTouch.screenY - o) >
				c.jGestures.defaults.thresholdMove
				? true
				: false;
		var B =
				...
				case "swipeleftup":
					if (x && y === true && B === true) {
						...
						q.type = [
							"swipe",
							q.delta[0].lastX != 0
								? q.delta[0].lastX > 0 ? "right" : "left"
								: "",
							q.delta[0].lastY != 0
								? q.delta[0].lastY > 0 ? "down" : "up"
								: ""
						].join("");
						v.triggerHandler(q.type, q);
					}
			...
			switch (n) {
				case "pinch":
					...
					if (
						(p < 1 &&
							p % 1 <
								1 - c.jGestures.defaults.thresholdPinchclose) ||
						(p > 1 &&
							p % 1 > c.jGestures.defaults.thresholdPinchopen)
					) {
						m = p < 1 ? -1 : +1;
						o = d({
							type: "pinch",
				...
				case "rotate":
					...
					if (
						(p < 1 &&
							-1 * p > c.jGestures.defaults.thresholdRotateccw) ||
		...
		for (m in o) {
			switch (m) {
				case "pinchclose":
					...
					if (
						n < 1 &&
						n % 1 < 1 - c.jGestures.defaults.thresholdPinchclose
					) {
						k.triggerHandler(
							"pinchclose",
							d({
				...
				case "pinchopen":
					...
					if (
						n > 1 &&
						n % 1 > c.jGestures.defaults.thresholdPinchopen
					) {
						k.triggerHandler(
							"pinchopen",
							d({
				...
				case "rotateccw":
					...
					if (
						n < 1 &&
						-1 * n > c.jGestures.defaults.thresholdRotateccw
					) {
						k.triggerHandler(
							"rotateccw",
							d({
