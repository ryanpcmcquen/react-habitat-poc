		...
		zoomMethods = {
			init: function(opts) {
				...
				for (eventName in data.zoom.flipbookEvents) {
					if (
						Object.prototype.hasOwnProperty.call(
							data.zoom.flipbookEvents,
							eventName
						)
					) {
						options.flipbook.bind(
							eventName,
							data.zoom.flipbookEvents[eventName]
						);
					}
				}
			...
			_eventSeq: function() {
				...
				if (
					...
					list[lastEvent].event.pageX ==
						list[lastEvent - 1].event.pageX &&
					list[lastEvent].event.pageY ==
						list[lastEvent - 1].event.pageY &&
					list[lastEvent].timestamp - list[lastEvent - 1].timestamp <
						200 &&
					list[lastEvent].timestamp - list[lastEvent - 1].timestamp >
						50
				) {
					...
					for (var p = 0; p < view.length; p++) {
						...
						pageElement
							...
							.css({
								...
								left:
									(pos.left - flipOffset.left - offsetLeft) *
									invz
							})
			...
			zoomIn: function(event) {
				...
				var compose = point2D(
					...
					move = point2D(
						bound.size.width * zoom > this.width()
							? pos.x - compose.x
							: 0,
						bound.size.height * zoom > this.height()
							? pos.y - compose.y
							: 0
					),
					maxMove = point2D(
				...
				data.zoomer.transform(
					translate(move.x, move.y, true) + scale(zoom, true)
				);
			...
			zoomOut: function(duration) {
				...
				var transitionEndCallback = function() {
					if (flip[0].style.removeProperty) {
						...
						flip
							.transform(
								flip.turn("options").acceleration
									? translate(0, 0, true)
									: ""
							)
							.turn("zoom", 1);
				...
				if (duration === 0) {
					...
					flip.transform(
						translate(move.x, move.y, true) + scale(scaling, true)
					);
				} else {
			...
			scroll: function(to, unlimited, animate) {
				...
				if (!unlimited) {
					var out,
						...
						maxBound = point2D(
							flipWidth > this.width()
								? flipWidth - this.width()
								: (flipWidth - this.width()) / 2,
			...
			resize: function() {
				...
				if (this.zoom("value") > 1) {
					...
					data.axis = point2D(
						flipOffset.left -
							thisOffset.left +
							(data.axis.x + data.scrollPos.x),
						flipOffset.top -
							thisOffset.top +
							(data.axis.y + data.scrollPos.y)
					);
			...
			_eTouchMove: function(event) {
				...
				if (data.touch && zoom == 1 && !flip.data().mouseAction) {
					...
					data.touch.speed.x =
						data.touch.speed.x === 0
							? data.touch.motion.x /
								(time - data.touch.timestamp)
							: (data.touch.speed.x +
									data.touch.motion.x /
										(time - data.touch.timestamp)) /
								2;
			...
			_eTouchEnd: function(event) {
				...
				if (data.touch && $(this).zoom("value") == 1) {
					...
					if (
						y < 50 &&
						(data.touch.speed.x < -1 ||
							data.touch.last.x - data.touch.initial.x < -100)
					) {
			...
			_eDestroying: function() {
				...
				flip
					.unbind("touchstart", data.eventTouchStart)
					.unbind("touchmove", data.eventTouchMove)
					.unbind("touchend", data.eventTouchEnd);

				this.unbind("touchstart", zoomMethods._tap).unbind(
					"click",
					zoomMethods._tap
				);
	...
	$.extend($.fn, {
		zoom: function() {
			...
			if (!args[0] || typeof args[0] == "object")
				...
				return zoomMethods[args[0]].apply(
					$(this[0]),
					Array.prototype.slice.call(args, 1)
				);
			else throw error(args[0] + " is not a method");
		}
	});
})(jQuery);
