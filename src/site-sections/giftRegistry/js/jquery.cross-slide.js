...
jQuery.fn.crossSlide = function(opts, plan) {
	...
	(function(proceed) {
		...
		function parse_position_param(param) {
			...
			if (tokens[0] == "center")
				if (tokens.length == 1) tokens = ["center", "center"];
			...
			if (pos == "center center")
				return { xrel: 0.5, yrel: 0.5, zoom: zoom };
		...
		function position_to_css(p, phase) {
			...
			return {
				...
				width: Math.round(
					p.from.width + pos * (p.to.width - p.from.width)
				),
				height: Math.round(
					p.from.height + pos * (p.to.height - p.from.height)
				)
			};
		}
		...
		if (opts.sleep) var sleep = Math.round(opts.sleep * 1000);
		if (opts.speed)
		...
		for (var i = 0; i < plan.length; ++i) {
			...
			if (speed) {
				...
				switch (p.dir) {
					...
					default:
						abort(
							'missing or malformed "dir" parameter in picture {0}.',
							i + 1
						);
				}
				if (slide_px <= 0)
					abort(
						"picture number {0} is too short for the desired fade duration.",
						i + 1
					);
				...
				if (!p.from || !p.to || !p.time)
					abort(
						"missing either speed/sleep option, or from/to/time params in picture {0}.",
						i + 1
					);
				try {
			...
			if (p.from)
				jQuery.each([p.from, p.to], function(i, from_to) {
					...
					from_to.left = Math.round(
						(self_width - from_to.width) * from_to.xrel
					);
					from_to.top = Math.round(
						(self_height - from_to.height) * from_to.yrel
					);
				});
			...
			else elm = jQuery(format('<img src="{0}"/>', p.src));
			if (p.onclick) elm.click(p.onclick);
			if (p.alt) elm.find("img").attr("alt", p.alt);
			elm.appendTo(self);
		}
		...
		if (!sleep) imgs.eq(0).css(position_to_css(plan[0], 2));
			...
			if (i % 2 == 0) {
				...
				if (sleep) {
					...
					var newf = function() {
						...
						else img_to.animate(to_anim, fade_ms, "linear", chainf);
					};
				} else {
