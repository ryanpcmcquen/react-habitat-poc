		...
		turnMethods = {
					...
					if (turnMethods._necessPage.call(this, page)) {
						if (!data.pageWrap[page]) {
							...
							if (!data.pagePlace[page]) {
								...
								data.pageObjs[page].appendTo(
									data.pageWrap[page]
								);
							}

							// Set the size of the page
							var prop = turnMethods._pageSize.call(
								this,
								page,
								true
							);
			...
			zoom: function(newZoom) {
				...
				if (typeof newZoom == "number") {
					...
					if (
						trigger("zooming", this, [newZoom, data.zoom]) ==
						"prevented"
					)
						return this;
        ...
        $(this).css({marginLeft: size.width * iz / 2 - newWidth / 2});*/
				...
				var page,
					...
					move = function(page) {
						...
						if (data.pagePlace[page] && data.pageWrap[page]) {
							...
							if (data.pages[page])
								data.pages[next] = data.pages[page].flip(
									"options",
									{
										page: next,
										next:
											single || odd ? next + 1 : next - 1
									}
								);
			...
			display: function(display) {
				...
				if (display === undefined) {
					...
					if (currentDisplay) {
						...
						this.turn("size", size.width, size.height).turn(
							"update"
						);
					}
			...
			size: function(width, height) {
				if (width === undefined || height === undefined) {
					...
					var page,
						...
						data = this.data(),
						pageWidth =
							data.display == "double" ? width / 2 : width;
			...
			_turnPage: function(page) {
				...
				if (data.page != page) {
					...
					if (
						trigger("turning", this, [page, newView]) == "prevented"
					) {
						if (
			...
			previous: function() {
				return this.turn(
					...
					Math.max(
						1,
						turnMethods._view.call(this, this.data().page).shift() -
							1
					)
				);
			},
			...
			peel: function(corner, animate) {
				...
				if (corner === false) {
					...
					if (data.display == "single") {
						...
						if (data.direction == "ltr") {
							page =
								corner.indexOf("l") != -1 ? view[0] : view[1];
						} else {
							page =
								corner.indexOf("l") != -1 ? view[1] : view[0];
						}
			...
			calculateZ: function(mv) {
				...
				for (i = 0; i <= total; i++) {
					...
					addView(nextPage);
					dpage =
						data.pagePlace[nextPage] == nextPage ? nextPage : page;
					r.pageZ[dpage] =
			...
			update: function() {
				...
				if (this.turn("animating") && data.pageMv[0] !== 0) {
					...
					for (page in data.pageWrap) {
						...
						var pageLocation = turnMethods._setPageLoc.call(
							this,
							page
						);
		...
		flipMethods = {
			...
			z: function(z) {
				...
				if (data.fwrapper)
					data.fwrapper.css({
						zIndex:
							z || parseInt(data.parent.css("z-index"), 10) || 0
					});
			...
			_addPageWrapper: function() {
				...
				if (!data.wrapper)
					switch (data.effect) {
						case "hard":
							var cssProperties = {};
							cssProperties[vendor + "transform-style"] =
								"preserve-3d";
							cssProperties[vendor + "backface-visibility"] =
								"hidden";
				...
				switch (data.effect) {
					case "hard":
						...
						if (turnData.opts.gradients) {
							...
							if (half) {
								if (
									data.bshadow.parent()[0] != data.wrapper[0]
								) {
						...
						var that = this,
							...
							compute = function() {
								...
								var alpha = A90 - Math.atan2(rel.y, rel.x),
									gamma =
										alpha - Math.atan2(middle.y, middle.x),
									distance = Math.max(
										0,
										Math.sin(gamma) *
											Math.sqrt(
								...
								if (alpha > A90) {
									tr.x =
										tr.x + Math.abs(tr.y * rel.y / rel.x);
									tr.y = 0;
									if (
										Math.round(
											tr.x * Math.tan(PI - alpha)
										) < height
									) {
										point.y = Math.sqrt(
											Math.pow(height, 2) +
								...
								if (turnData.opts.gradients) {
									...
									var endingPoint = flipMethods._c2.call(
										...
										far =
											Math.sqrt(
												Math.pow(
													endingPoint.x - point.x,
													2
												) +
													Math.pow(
														endingPoint.y - point.y,
														2
													)
											) / width;
									...
									if (flipMethods._backGradient.call(that)) {
										...
										if (!left)
											gradientEndPointB.x =
												100 - gradientEndPointB.x;
										if (!top)
											gradientEndPointB.y =
												100 - gradientEndPointB.y;
									}
								}
							...
							transform = function(tr, c, x, a) {
								...
								if (turnData.opts.gradients) {
									if (x[0])
										gradientEndPointA.x =
											100 - gradientEndPointA.x;

									if (x[1])
										gradientEndPointA.y =
											100 - gradientEndPointA.y;
			...
			_showFoldedPage: function(c, animate) {
				...
				if (folding) {
					if (
						...
						if (
							trigger("start", this, [data.opts, corner]) ==
							"prevented"
						)
							return false;
					}
			...
			peel: function(corner, animate) {
				...
				if (corner) {
					...
					if (
						$.inArray(corner, flipMethods._cAllowed.call(this)) !=
						-1
					) {
			...
			for (j = 0; j < numColors; j++)
				cols.push(
					" " +
						colors[j][1] +
						" " +
						(segA + gradientDiagonal * colors[j][0]) *
							100 /
							diagonal +
						"%"
				);
