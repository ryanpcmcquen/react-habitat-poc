...
(function($) {
	...
	(validators = {
		...
		url: function(value, el) {
			return /^https?:\/\/[\w\-_]+(\.[\w\-_]+)+([\w\-\.,@?^=%&amp;:\/~\+#]*[\w\-\@?^=%&amp;\/~\+#])?$/.test(
				value
			);
		},

		email: function(value, el) {
			return /^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,4}$/.text(value);
		},

		int: function(value, el) {
			return /^\d+$/.test(value);
		},

		float: function(value, el) {
		...
		duration: function(value, el) {
			return /^P(([0-9.,]+[YMD])*(T[0-9.,]+[HMS])*|[0-9.,]W)$/.test(
				value
			);
		},

		// TODO: complex types require other nested entities or special treatment
		complex: function(value, el) {
			return true;
		},
		...
		currency: function(value, el) {
			return /^[a-zA-Z]{3}$/.test(value);
		},

		any: function(value, el) {
			return true;
		},

		boolean: function(value, el) {
	...
	var validateData = function(mdata) {
		...
		required = $.grep(rules, function(item) {
			return item.required;
		});

		for (i = 0; i < mdata.properties.length; i++) {
			rule = $.grep(rules, function(item) {
				return item.name === mdata.properties[i].name.toLowerCase();
			});
			...
			if (
				rule.length > 0 &&
				!rule[0].validator(mdata.properties[i].value)
			) {
	...
	var parseElement = function(el) {
		...
		var propElements = el.find("[itemprop]"),
			props = [],
			i;
	...
	var addObject = function(element, mdata) {
		...
		mdata =
			typeof mdata.valid === "undefined"
				? (mdata = validateData(mdata))
				: mdata;
		...
		if (mdata.properties.length > 0) {
			for (i = 0; i < mdata.properties.length; i++) {
				prop = $(
					"<li>" +
						mdata.properties[i].name +
						" = " +
						mdata.properties[i].value +
						"</li>"
				).appendTo(u);
			...
			if (mdata.missing.length > 0) {
				for (i = 0; i < mdata.missing.length; i++) {
					$(
						'<li class="invalid">missing property: ' +
							mdata.missing[i] +
							"</li>"
					).appendTo(u);
				}
		...
		t
			.hover(
				function() {
					$(element).addClass("microdata-highlighted");
				},
				function() {
					$(element).removeClass("microdata-highlighted");
				}
			)
			.click(function() {
	...
	var updateList = function() {
		...
		if (items.length === 0) {
			$(
				'<li class="invalid">No microdata objects detected!</li>'
			).appendTo(widget);
			return;
		}

		items.each(function() {
			addObject(this, parseElement(this));
		});
	};
	...
	$.fn.hasAttr = function(name) {
		return (
			typeof this.attr(name) !== "undefined" || this.attr(name) !== false
		);
	};
